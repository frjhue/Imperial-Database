...
const pool = new Pool({
    ...
    database: process.env.DB_NAME || 'inquisition',
    ...
    try {
        ...
        await pool.query(`
            CREATE TABLE IF NOT EXISTS imperial_casefiles (
                ...
                status VARCHAR(50),
                assigned_officer INTEGER REFERENCES personnel(id),
        ...
        await pool.query(`
            INSERT INTO personnel (id, callsign, password, rank, clearance_level, department, status, created_at) 
...
async function logAction(action, user = 'System', details = '') {
    ...
    try {
        await pool.query(
            'INSERT INTO logs (timestamp, actor_name, action, target_type, target_id) VALUES ($1, $2, $3, $4, $5)',
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'SELECT * FROM personnel WHERE callsign = $1 AND password = $2',
            [callsign, password]
        );
...
app.get('/api/auth/verify', authenticate, async (req, res) => {
    try {
        const result = await pool.query(
            'SELECT * FROM personnel WHERE id = $1 AND status = $2',
            [req.user.id, 'active']
        );
...
app.post('/api/subjects', authenticate, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'INSERT INTO subjects (designation, classification, planet_of_origin, loyalty_status, notes, created_at) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
...
app.put('/api/subjects/:id', authenticate, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'UPDATE subjects SET designation = $1, classification = $2, planet_of_origin = $3, loyalty_status = $4, notes = $5, updated_at = $6 WHERE id = $7 RETURNING *',
...
app.get('/api/casefiles', authenticate, async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM casefiles ORDER BY id DESC');
        res.json(result.rows);
    } catch (err) {
        return res.status(500).json({ error: 'Database error' });
    }
...
app.get('/api/casefiles/:id', authenticate, async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM casefiles WHERE id = $1', [req.params.id]);
        if (result.rows.length === 0) {
...
app.put('/api/casefiles/:id', authenticate, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'UPDATE casefiles SET designation = $1, threat_level = $2, status = $3, assigned_officer = $4, summary = $5, content = $6, access_level = $7, updated_at = $8 WHERE id = $9 RETURNING *',
...
app.delete('/api/casefiles/:id', authenticate, async (req, res) => {
    const id = req.params.id;
    try {
        const result = await pool.query('DELETE FROM casefiles WHERE id = $1 RETURNING id', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Case not found' });
        }
...
app.post('/api/reports', authenticate, async (req, res) => {
    ...
    try {
        ...
        const caseResult = await pool.query('SELECT designation FROM casefiles WHERE id = $1', [case_id]);
        const case_designation = caseResult.rows.length > 0 ? caseResult.rows[0].designation : `Case_${case_id}`;
        
        const result = await pool.query(
            'INSERT INTO reports (case_id, case_designation, author_name, content, classification, access_level, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
        ...
        await pool.query('UPDATE casefiles SET report_count = report_count + 1 WHERE id = $1', [case_id]);
...
app.get('/api/evidence', authenticate, async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM evidence ORDER BY id DESC');
...
app.post('/api/evidence', authenticate, async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            'INSERT INTO evidence (case_id, case_designation, file_name, storage_path, evidence_type, uploaded_by_name, access_level, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
...
app.delete('/api/evidence/:id', authenticate, async (req, res) => {
    const id = req.params.id;
    try {
        const result = await pool.query('DELETE FROM evidence WHERE id = $1 RETURNING id', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Evidence not found' });
        }
...
app.get('/api/entities', authenticate, async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM entities ORDER BY id DESC');
        res.json(result.rows);
    } catch (err) {
        return res.status(500).json({ error: 'Database error' });
    }
...
app.post('/api/entities', authenticate, async (req, res) => {
    ...
    try {
        ...
        if (linked_subject_id) {
            const subjectResult = await pool.query('SELECT designation FROM subjects WHERE id = $1', [linked_subject_id]);
            if (subjectResult.rows.length > 0) subject_name = subjectResult.rows[0].designation;
        }
        
        if (linked_case_id) {
            const caseResult = await pool.query('SELECT designation FROM casefiles WHERE id = $1', [linked_case_id]);
            if (caseResult.rows.length > 0) case_name = caseResult.rows[0].designation;
        }
        
        const result = await pool.query(
            'INSERT INTO entities (entity_name, entity_type, classification, description, subject_name, case_name, threat_rating, access_level, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *',
...
app.get('/api/personnel', authenticate, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'SELECT id, callsign, rank, clearance_level, department, status, created_at FROM personnel ORDER BY id DESC'
        );
        res.json(result.rows);
    } catch (err) {
        return res.status(500).json({ error: 'Database error' });
...
app.get('/api/personnel/:id', authenticate, async (req, res) => {
    try {
        const result = await pool.query(
            'SELECT id, callsign, rank, clearance_level, department, status, created_at FROM personnel WHERE id = $1',
            [req.params.id]
        );
        if (result.rows.length === 0) {
...
app.post('/api/personnel', authenticate, requireClearance(900), async (req, res) => {
    ...
    try {
        ...
        const existing = await pool.query('SELECT id FROM personnel WHERE callsign = $1', [callsign]);
        ...
        const result = await pool.query(
            'INSERT INTO personnel (callsign, password, rank, clearance_level, department, status, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id, callsign, rank, clearance_level, department, status, created_at',
...
app.put('/api/personnel/:id/toggle', authenticate, requireClearance(900), async (req, res) => {
    ...
    try {
        const personResult = await pool.query('SELECT status FROM personnel WHERE id = $1', [id]);
        if (personResult.rows.length === 0) {
            return res.status(404).json({ error: 'Personnel not found' });
        }
        
        const newStatus = personResult.rows[0].status === 'active' ? 'inactive' : 'active';
        await pool.query('UPDATE personnel SET status = $1 WHERE id = $2', [newStatus, id]);
...
app.get('/api/logs', authenticate, async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM logs ORDER BY id DESC LIMIT 200');
        res.json(result.rows);
    } catch (err) {
        return res.status(500).json({ error: 'Database error' });
    }
